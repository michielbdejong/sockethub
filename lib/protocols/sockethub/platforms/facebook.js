...
var Facebook = function () {
  ...
  o.fetch = function (job) {
    var promise = promising();
    session.log('facebook fetch called');
    session.getConfig('credentials').then(function (credentials) {
      ...
      var req = https.request({
        ...
        path: '/'+ job.actor.address +'/'+ job.target +'?access_token='+ encodeURIComponent(creds.access_token),
        method: 'GET'
      }, function (res) {
        ...
        res.on('end', function() {
          ...
          session.log('does response have error? ' + typeof data.error);
